...
(function ($) {
    ...
    function updateRadio(radioId) {
        ...
        // console.log('off for while')
    }

    function ajax_theme(val)
    {
        ajaxcsrf()
        $.ajax({
            url:site+'settings/ajax_theme',
            method:'POST',data:{val:val},
            beforeSend: function() {
            ...
            success:function(data)
            {
                setTimeout(function() {
                    swal.close()
                    alertify.success('Berhasil menyimpan pengaturan');
                }, 500);
            },
            error:function()
            {
                setTimeout(function() {
                    swal.close();
                    alertify.error('Gagal menyimpan pengaturan');
                }, 500);
            }
        });
    }
