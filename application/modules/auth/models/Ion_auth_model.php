...
class Ion_auth_model extends CI_Model
{
	...
	public function deactivate($id = NULL)
	{
		...
		$random_code = sha1(md5(microtime()));

		$activation_code	= $random_code;
		$active_code		= strtoupper(substr($random_code, 10, 6));
	...
	public function login($identity, $password, $remember = FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', username, first_name, last_name, email, id, password, active, last_login, created_on, ip_address')
			->where($this->identity_column, $identity)
			->limit(1)
			->order_by('id', 'desc')
			->get($this->tables['users']);
	...
	/* Google OAuth2 - Register | Activation | Login */
	public function google_register($identity, $password, $email, $data)
	{
	...
	public function google_activate($id)
	{
		if (isset($id)) {
			$data = array(
				'activation_code' => NULL,
	...
	public function generate_activation($id)
	{
		if (isset($id)) {
			...
			$activation_code	= $random_code;
			$active_code		= strtoupper(substr($random_code, 6, 6));
